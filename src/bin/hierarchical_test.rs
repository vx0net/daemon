...
use vx0net_daemon::node::{PeerConnection, ServiceType, ServiceStatus, HostedService, NodeTier};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let backbone1 = create_test_node("backbone1.vx0", 65001, "10.0.1.1", NodeTier::Backbone).await?;
    let backbone2 = create_test_node("backbone2.vx0", 65002, "10.0.1.2", NodeTier::Backbone).await?;
    
    // Create regional nodes (ASN 65100-65999)
    let regional1 = create_test_node("regional1.vx0", 65101, "10.1.1.1", NodeTier::Regional).await?;
    let regional2 = create_test_node("regional2.vx0", 65102, "10.1.2.1", NodeTier::Regional).await?;
    ...
    backbone1.add_peer(PeerConnection::new(backbone2.node_id, backbone2.asn, backbone2.ipv4_addr.into())).await?;
    backbone2.add_peer(PeerConnection::new(backbone1.node_id, backbone1.asn, backbone1.ipv4_addr.into())).await?;
    println!("    ✅ Backbone1 ↔ Backbone2");
    
    // Backbone to regional  
    backbone1.add_peer(PeerConnection::new(regional1.node_id, regional1.asn, regional1.ipv4_addr.into())).await?;
    regional1.add_peer(PeerConnection::new(backbone1.node_id, backbone1.asn, backbone1.ipv4_addr.into())).await?;
    
    backbone2.add_peer(PeerConnection::new(regional2.node_id, regional2.asn, regional2.ipv4_addr.into())).await?;
    regional2.add_peer(PeerConnection::new(backbone2.node_id, backbone2.asn, backbone2.ipv4_addr.into())).await?;
    println!("    ✅ Backbone → Regional connections");
    
    // Regional to regional
    regional1.add_peer(PeerConnection::new(regional2.node_id, regional2.asn, regional2.ipv4_addr.into())).await?;
    regional2.add_peer(PeerConnection::new(regional1.node_id, regional1.asn, regional1.ipv4_addr.into())).await?;
    println!("    ✅ Regional1 ↔ Regional2");
    
    // Regional to edge
    regional1.add_peer(PeerConnection::new(edge1.node_id, edge1.asn, edge1.ipv4_addr.into())).await?;
    edge1.add_peer(PeerConnection::new(regional1.node_id, regional1.asn, regional1.ipv4_addr.into())).await?;
    
    regional1.add_peer(PeerConnection::new(edge2.node_id, edge2.asn, edge2.ipv4_addr.into())).await?;
    edge2.add_peer(PeerConnection::new(regional1.node_id, regional1.asn, regional1.ipv4_addr.into())).await?;
    
    regional2.add_peer(PeerConnection::new(edge3.node_id, edge3.asn, edge3.ipv4_addr.into())).await?;
    edge3.add_peer(PeerConnection::new(regional2.node_id, regional2.asn, regional2.ipv4_addr.into())).await?;
    println!("    ✅ Regional → Edge connections");

    // Test invalid peering (edge to edge should fail)
    println!("  Testing peering restrictions...");
    match edge1.add_peer(PeerConnection::new(edge2.node_id, edge2.asn, edge2.ipv4_addr.into())).await {
    ...
    bgp_backbone1.add_route(vx0_default, "10.0.1.1".parse()?, BGPOrigin::IGP).await?;
    bgp_backbone2.add_route(vx0_default, "10.0.1.2".parse()?, BGPOrigin::IGP).await?;
    ...
    bgp_regional1.add_route(region1_net, regional1.ipv4_addr.into(), BGPOrigin::IGP).await?;
    bgp_regional2.add_route(region2_net, regional2.ipv4_addr.into(), BGPOrigin::IGP).await?;
    ...
    bgp_edge1.add_route(edge1_services, edge1.ipv4_addr.into(), BGPOrigin::IGP).await?;
    bgp_edge2.add_route(edge2_services, edge2.ipv4_addr.into(), BGPOrigin::IGP).await?;
    bgp_edge3.add_route(edge3_services, edge3.ipv4_addr.into(), BGPOrigin::IGP).await?;
    ...
    println!("  Backbone1 routes ({}): Full internet table", backbone1_routes.len());
    ...
    println!("  Regional1 routes ({}): Regional + backbone", regional1_routes.len());
    for route in &regional1_routes {
        println!("    {} via {} (AS: {:?})", route.network, route.next_hop, route.as_path);
    }

    let edge1_routes = bgp_edge1.get_routes().await;
    println!("  Edge1 routes ({}): Default + local only", edge1_routes.len());
    ...
    edge1.register_service(HostedService {
    ...
    edge2.register_service(HostedService {
        service_id: uuid::Uuid::new_v4(),
    ...
    edge3.register_service(HostedService {
        service_id: uuid::Uuid::new_v4(),
    ...
    println!("    Nodes: 2");
    println!("    Peers: {} + {}", backbone1.get_peer_count().await, backbone2.get_peer_count().await);
    println!("    Routes: {} + {}", backbone1_routes.len(), bgp_backbone2.get_routes().await.len());
    
    println!("  Regional Tier:");
    println!("    Nodes: 2");
    println!("    Peers: {} + {}", regional1.get_peer_count().await, regional2.get_peer_count().await);
    println!("    Routes: {} + {}", regional1_routes.len(), bgp_regional2.get_routes().await.len());
    
    println!("  Edge Tier:");
    println!("    Nodes: 3");
    println!("    Peers: {} + {} + {}", edge1.get_peer_count().await, edge2.get_peer_count().await, edge3.get_peer_count().await);
...
async fn create_test_node(hostname: &str, asn: u32, ip: &str, tier: NodeTier) -> Result<Arc<Vx0Node>, Box<dyn std::error::Error>> {
