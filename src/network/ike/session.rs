use crate::network::ike::{IKESession, IKEError, IKEState};
...
impl IKEDaemon {
    ...
    async fn handle_packet(data: &[u8], sender: SocketAddr) -> Result<(), IKEError> {
        ...
        tracing::debug!("Processing IKE packet from {}, {} bytes", sender, data.len());
...
impl IKESession {
    pub async fn send_message(&self, message: &[u8]) -> Result<(), IKEError> {
        ...
        tracing::debug!("Sent IKE message to {} ({} bytes)", self.peer_addr, message.len());
        Ok(())
    }
